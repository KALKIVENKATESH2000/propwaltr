...
exports.verifyOTP = async (req, res) => {
  try {
    ...
    // if(result.status === "approved") {
      ...
      const resultUserByPhone = await mysqlPool.queryWithValues('select * from user where phone = ? and type = "service_provider"', [phone])
      ...
      if(resultUserByPhone.result.length === 0) {
        ...
        const databaseValues = [user_id, phone, 'service_provider', '', '', '', '', 1, 0, 0, 0, '', '', util.currentTimestamp(), util.currentTimestamp()];
        ...
        const resultUseInfoAdded = await mysqlPool.queryWithValues('INSERT INTO service_provider_info (`user_id`)  VALUES (?)', [[user_id]]);
